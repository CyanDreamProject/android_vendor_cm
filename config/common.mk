...
ifeq ($(CD_BUILDTYPE), RELEASE)
    CD_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(CD_BUILD)
else
    ifeq ($(PRODUCT_VERSION_MINOR),0)
        CD_VERSION := $(PRODUCT_VERSION_MAJOR)-$(shell date -u +%Y%m%d)-$(CD_BUILDTYPE)$(CD_EXTRAVERSION)-$(CD_BUILD)
    else
        CD_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d)-$(CD_BUILDTYPE)$(CD_EXTRAVERSION)-$(CD_BUILD)
    endif
endif
